...
function wrap_pvcam_fun!(ctx::AbstractContext, cursor::CLFunctionDecl)
    ...
    body = eccall(func_name, Symbol("libpvcam"), ret_type, arg_names, arg_reps)
    push!(ctx.api_buffer, Expr(:function, signature, Expr(:block, body)))

    return ctx
end

function wrap_pvcam_struct!(ctx::AbstractContext, cursor::CLStructDecl)
